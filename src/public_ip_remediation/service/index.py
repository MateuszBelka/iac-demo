...
def lambda_handler(event, _):
    ...
    if instance_state == "terminated":
        ...
        return {"Status": "SUCCESS", "Message": f"Instance {instance_id} is terminated - no remediation needed"}

    public_ip = instance.get("PublicIpAddress")
    if not public_ip:
        logger.info(f"Instance {instance_id} does not have a public IP")
        return {"Status": "SUCCESS", "Message": f"Instance {instance_id} does not have a public IP - already compliant"}
    ...
    if association and association.get("AssociationId"):
        ...
        logger.info(f"Successfully disabled auto-assign public IP on subnet {subnet_id}")
        
        if instance_state == "running":
            logger.info(f"Stopping instance {instance_id} to remove auto-assigned public IP")
